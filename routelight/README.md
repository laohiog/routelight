...
RouteLight 旨在以轻量、只读、低打扰的方式，为用户提供周期性更新的网络环境视图。它通过后台轻量端点握手，在一个系统托盘图标中竖排显示三条横向圆角状态带，从上到下固定对应 `ChatGPT`、`Claude` 和 `Google AI`：

* 绿色：当前探测可达或可用
* 黄色：端点可达但响应异常，或需要人工确认
* 红色：当前探测不可达，或 Google AI 明确提示地区不支持
* 灰色：尚未完成检测或当前结果无法可靠判断

托盘悬停提示会按相同顺序列出三个服务，并额外显示总体路由状态。IPv4 / IPv6 出口、CN 风险等非 AI 诊断仍会影响总体状态、系统通知和主面板，但不会改变三条服务状态带的颜色。
...
2. **AI 服务连通性与 Google AI 可用性检测**
   周期性检测 `ChatGPT` 与 `Claude` 的端点连接延迟和 HTTP 状态，并通过无账号、无 Cookie 的匿名请求判断当前网络出口是否可进入 Google AI Mode。Google AI 结果区分“可用”“地区不支持”“需人工确认”“不可达”和“无法判定”。
...
* 🚫 **不上传诊断报告**
  RouteLight 会向固定公网检测端点发起连通性探测请求，例如出口 IP、Geo-IP、`chatgpt.com`、`claude.ai` 和 `google.com/ai`；但不会上传诊断报告、代理配置、浏览器 Cookie、账号信息、剪贴板内容、本地文件或用户自定义数据。
...
$env:ROUTELIGHT_MOCK_STATUS="normal"; npm run tauri dev   # 绿 / 绿 / 绿
$env:ROUTELIGHT_MOCK_STATUS="warning"; npm run tauri dev # 绿 / 红 / 绿（Claude 不可达）
$env:ROUTELIGHT_MOCK_STATUS="error"; npm run tauri dev   # 红 / 红 / 红
$env:ROUTELIGHT_MOCK_STATUS="unknown"; npm run tauri dev # 灰 / 灰 / 灰
```
...
* NSIS 安装包：`src-tauri/target/release/bundle/nsis/RouteLight_0.3.0_x64-setup.exe`
  或匹配通配符：`RouteLight_*_x64-setup.exe`

v0.3.0 默认仅构建并推荐分发 NSIS 安装包。MSI / WiX 打包链路暂不作为 v0.3.0 推荐分发产物；如后续需要 MSI，应单独修复并验证 WiX `light.exe` 打包流程后再启用。

当前前端是无 bundler 的静态 HTML / CSS / JS，并通过 Tauri 注入的 `window.__TAURI__` 调用 `invoke`、`listen` 与窗口拖拽 API，因此 v0.3.0 暂时保留 `withGlobalTauri`。同时，Tauri 配置已启用最小 CSP，限制脚本、样式、图片和 IPC 连接来源。
...
* **端点检测受限性**
  AI 端点检测是对目标域名进行实际 HTTPS 握手测试。托盘绿灯只代表最近一次探测在当前网络路径下可达或可用，不代表账号登录、对话功能、API 或第三方服务的所有能力均正常。部分接口可能受限于本地网络服务商的临时解析状况、目标服务的 CDN 调度、网关策略或速率限制。

* **Google AI 判定是匿名网络视角**
  Google AI 检测依据 `google.com/ai` 的最终跳转地址和明确的地区限制页面进行判断，不登录 Google 账号，也不读取浏览器 Cookie。账号类型、语言、实验分组、设备位置或验证码仍可能导致浏览器中的实际结果不同；遇到此类响应时 RouteLight 会显示“需人工确认”或“无法判定”，不会武断地判为地区不支持。
